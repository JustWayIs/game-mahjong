 ...
 * @Declare:
 */
public enum QingJinGouDiaoFan implements Fan<CompoundFanParam> {
    /**
     * 单例
     */
    INSTANCE(CompoundFanTypeEnum.清金钩钓);

    public FanType fantype;

    QingJinGouDiaoFan(FanType fantype) {
    ...
    public FanType judge(CompoundFanParam param) {
        ...
        if(BaseHuTypeEnum.平胡.equals(baseHuType)){
            ...
            for(FanType fanType : param.getFanTypeList()){
                ...
                if(FormalFanTypeEnum.金钩钓.equals(fanType)){
                    num++;
                }
            }
            if(num == 2){
