...
public class SichuanPlayHelper {
    ...
    public static void main(String[] args) {
        List<Integer> cards = new ArrayList<>(Arrays.asList(12, 25, 36, 11, 16, 18, 28, 19, 19, 19, 31, 33, 28, 29));
        sortStandCardList(cards,1);
    ...
    public static void sortStandCardList(List<Integer> standCardList, Integer dingQueColor) {
        Collections.sort(standCardList, (beforeCard, afterCard) -> {
            if (CardEnum.judgeCardColor(beforeCard).getColor().equals(dingQueColor)) {
                beforeCard += 100;
            }
            if(CardEnum.judgeCardColor(afterCard).getColor().equals(dingQueColor)){
                afterCard += 100;
            }
            if (beforeCard > afterCard) {
                return 1;
            } else if (beforeCard.equals(afterCard)) {
                return 0;
            } else {
                return -1;
            }
